...
public class Parser {
    ...
    private Stmt ifStatement() {
        ...
        return new Stmt.If(condition, thenBranch, match(TokenType.ELSE) ? statement() : null);
    }
    ...
    private Expr assignment() {
        ...
        if (match(TokenType.EQUAL)) {
            ...
            //  We report an error if the left-hand side isn’t a valid assignment target, but we don’t throw it because
        ...
        if (!check(TokenType.RIGHT_PAREN)) {
            do {
                if (arguments.size() >= 255) {
                    ...
                    // But here, the parser is still in a perfectly valid state — it just found too many arguments.
                    // So it reports the error and keeps on keepin' on.
                    // noinspection ThrowableNotThrown
